...
- `main.py`：命令行入口，提供 demo 与 MQTT 两种运行模式，并完成配置解析与适配器装配。
- `hb.py`：心跳/遗嘱占位文件，后续将补充 LWT 与健康检查逻辑。

## 关键职责

- **Actor**：读取命令队列、调用驱动、产出进度/完成事件，同时捕获异常并转换为 `ErrorEvent`。
- **队列**：在 Actor 与 MQTT 适配器之间做缓冲，支持 backpressure 与异步解耦。
- **入口**：统一处理配置与运行模式，保证测试、CLI、部署阶段都能重用相同装配流程。

## 开发约定

- 异步循环需支持 `stop()`，以便 CLI 和测试能够优雅退出。
- 新增入口模式优先收敛在 `main.py`，保持调用一致性。
- 配置字典交由 `create_actor` 统一处理，方便在不同模式间复用。
